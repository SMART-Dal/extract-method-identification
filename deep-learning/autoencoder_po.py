...
def train_autoencoder(data, batch_size, num_epochs,n_inputs,encoding_dim, save_interval=1, valid_data=None, model_shorthand="gc"):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % save_interval == 0:
            checkpoint_path = f"./trained_models/autoencoder_{model_shorthand}_po_{encoding_dim}_{epoch + 1}.pth"
            torch.save(autoencoder.state_dict(), checkpoint_path)
            print(f"Model checkpoint saved at epoch {epoch + 1}")
    
    # Save losses list
    with open(f'train_losses_{encoding_dim}_po.pkl', 'wb') as f:
        pickle.dump(train_losses, f)

    with open(f'val_losses_{encoding_dim}_po.pkl', 'wb') as f:
...
if __name__=="__main__":
    ...
    train_autoencoder(train_data,8,150,768,128,save_interval=10, valid_data=valid_data,model_shorthand="gc")